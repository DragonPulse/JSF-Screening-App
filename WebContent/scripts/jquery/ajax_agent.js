...
$("#br1").hide();
	...
	/* function getParams() {
		...
		if(date1=="null")
			...
			window.location.replace("welcomeagent.do");
...
// step 22222222222
function ajaxFunction2(from,to,date,vv)
{
...
check=vv;
	  ...
	  if(xmlhttp) {
		     ...
		     xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
			    //alert("name: "+name);
			    dataType: "json";
			    xmlhttp.send('action=Booking/searchBus&userName='+name+'&sourceName='+sourceId+'&destinationName='+destinationId+'&journeyDate='+journeyDate+'&User=3');
            ...
            for (var i = 0; i < 5; i++) {
            	...
            	if(i>-1){
            		if(i==0)
            		{
                		...
                		radioInput.setAttribute('name', 'name11');
...
//alert("pgm here...."+check);
	...
	var xmlhttp = new getXMLObject();	
	var my=0,src,dest,dat,rid,far,busType2,check_s;
	function ajaxFunctions3(sourceId,destinationId,date,routeId,fare,checkv,busType)
	{
		...
		far=fare;
		 var url;
		 check=checkv;
		 var splitData=busType2.split(" ");
		 check_s=busType2;
			var isSleeperBus=false;
	...
	function handle1() 
	{
		if (xmlhttp.readyState == 4) {
		    ...
		    //alert("ttttttt=="+busType2);
		    	 		ajaxFunction11(src,dest,rid,far,check,busType2);
	...
	function ajaxFunction(sourceId1,destinationId1,journeyDate1,busId1,fare1,check,busType)
	{
		...
		for(var s=0;s<splitData.length;s++)
		  ...
		  if(xmlhttp) {
				  var name=document.getElementById("u_name").value;//document.getElementById("u_name").value;//'admin';
				  //alert("namessss: "+name);
		 ...
		 my= xmlhttp.onreadystatechange  = function()
		 {
			   if (xmlhttp.readyState == 4) {
			    ...
			    var exe=document.getElementById("execuseme").value;
				 //alert("exe"+exe);
			    dataType: "json";
			    xmlhttp.send('action=Booking/getAdminBusLayout&userName='+name+'&sourceName='+sourceId+'&destinationName='+destinationId+'&journeyDate='+journeyDate+'&routeId='+busId+'&execuseme='+exe);
			  }
	}
	//sleeper-select
	function sleeper_selected(m11) {
    	...
    	/*var m=String(m11);
    	var mm=m.substring(0,0);
    	alert(mm);*/
    	//alert(m11+" "+seat_counter);
    	//alert("seat_counter=="+seat_counter);
    	if(seat_counter<6)
    	{//alert("comes "+book_seats.length);
    		//alert("book_seats="+book_seats)
						    									...
						    									if(m==agent_blocked[block])
						        			   					{
									        		  			        ...
									        		  			        s_cell.setAttribute("onclick","alert('You do not have permission to book this seat.');");//+seat_no+
							        		  			   		   	}
							        		  			   		   	else
							        		  			   		   		{
									        		  			   			s_cell.style.backgroundImage="url("+img_block+")";
									        		  			   			s_cell.setAttribute("onclick","alert('You do not have permission to book this seat.');");
							        		  			   		   		}
							        		  			   		   	s_cell.style.backgroundRepeat="no-repeat";
						    		...
						    		inc=inc+1;
						    	 ...
						    	 else
						    		 {
						    			...
						    			else if(last_blocked==true)//last_blocked
					    				{
					    					...
					    					cells.setAttribute("onclick", "alert('You do not have permission to book this seat.')");
					    				}
						    			else
						    					{
    ...
    function start(booked_seatsss,ladies,blck,fare,board,route,check,popup) {
    	//alert("check value in start"+check);
    	$("#br").hide();
	$("#br1").hide();
    	agent_blocked=new Array();
    	book_seats=new Array();
    									...
    									if(m==agent_blocked[block])
        			   					{
        		  			 				...
        		  			 				cell.setAttribute("onclick","block();");
        		  			   		   	}
        		  			   		   	else{
										cell.style.backgroundImage="url("+img_block+")";
...
// set number to bus seats	
    ...
    function selected(m) {
        ...
        var arr,len;
        function seats_list(val)//booking
        {
        	...
        	if(len>=1)
        	{
        		...
        		if(arr.length>0){var boarde="";
        			...
        			//alert("length: "+boarded1[0]+boarded1[1]+boarded1[2]);
        			for(var y=0;y<(boarded1.length)-1;y++)
        				{
        				if((boarded1.length)-2==y)
        					{//alert("if cont:"+boarded1[y]);
...
*/        		}
        	...
        	//  alert("seat number: "+num);  
        	  ...
        	  if(num<10){
        		for(var m=0;m<=lady.length;m++){
        			if(num==lady[m]){
        	  		...
        	  		break;
    					...
    					num=sleeper_seats[num-1];
        				 ...
        				 newdiv.innerHTML = '<div  style="margin-left:3%;"><table border="0"><tr><td width="25%"><span style="font-size:12px;margin-top:2%;">'+num+'</span></td><td><lable style="margin-left:11%;">&nbsp;&nbsp;<select class="logg" id="gender'+num+'"style="font-size: 10px;width:120px;height:24px;"><option>--SELECT--</option><option>MALE</option><option>FEMALE</option></select></lable><input type="hidden" style="width:28px;height:15px;" value="0" value="" maxlength="2" id="age'+num+'"></td></tr></table></div>';
        				}
        			else{ continue;}
        		}
        	  }
        		  ...
        		  newdiv.innerHTML = '<div  style="margin-left:2%;"><table border="0"><tr><td width="25%"><span style="font-size:12px;margin-top:2%;">'+num+'</span></td><td><lable style="margin-left:11%;"><select class="logg" id="gender'+num+'" style="font-size: 10px;width:120px;height:24px;"><option>--SELECT--</option><option>FEMALE</option></select></lable><input type="hidden" style="width:28px;height:15px;" value="0" value="" maxlength="2" id="age'+num+'"></td></tr></table></div>';
        		  break;
        			}
        			else if(m==((lady.length-1)))
        				{
        				...
        				newdiv.innerHTML = '<div  style="margin-left:2%;"><table border="0"><tr><td width="25%"><span style="font-size:12px;margin-top:2%;">'+num+'</span></td><td><lable style="margin-left:9%;"><select class="logg" id="gender'+num+'" style="font-size: 10px;width:120px;height:24px;"><option>--SELECT--</option><option>MALE</option><option>FEMALE</option></select></lable><input type="hidden" style="width:28px;height:15px;" value="0" maxlength="2" value="" id="age'+num+'"></td></tr></table></div>';	
        				}
        			else{ continue;}
        		}
        	  ...
        	  ik++;//alert("value"+document.getElementById("gender8").value);
        	  disp(ik);
     ...
     /* function  pwd_length()
        {
        	...
        	var n_pwd=document.getElementById("n_password").value;
        	var c_pwd=document.getElementById("c_password").value;
        	var ex=document.getElementById("execuseme1").value;
        	
        	if(pwd.length>7 && c_pwd.length>7 && n_pwd.length>7 && pwd.length<21 && c_pwd.length<21 && n_pwd.length<21)
        		{
        				if(n_pwd==c_pwd)
        					{
        						if(n_pwd==pwd){
        							...
        							document.getElementById("error1").innerHTML="please enter correct new password";
	...
	function ticket_book()
	{
    	...
    	var userName=document.getElementById("u_name").value;//username,u_name
	...
	if (r==true)
	  {
		window.location.replace("agentbusdetails.do?from="+a_from+"&&to="+a_to+"&&txtEighth="+comp);
	  }

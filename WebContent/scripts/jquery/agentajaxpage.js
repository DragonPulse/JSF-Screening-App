...
function page(f, t, ddd, aa) {
	...
	document.getElementById("date").innerHTML = date1.split("/")[1] + "/"
			+ date1.split("/")[0] + "/" + date1.split("/")[2];
	if (date1 == "null") {
...
function mod_date(op, op1) {
	...
	if (op == 1) {
		if (d1.getDate() <= ds.getDate() && d1.getMonth() <= ds.getMonth()) {
	...
	document.getElementById("date").innerHTML = new_da.split("/")[1] + "/"
			+ new_da.split("/")[0] + "/" + new_da.split("/")[2];
...
function ajaxFunction2(from, to, date, vv, op1) {
	...
	if (xmlhttp) {
		...
		xmlhttp.send('action=Booking/searchBus&userName=' + name
				+ '&sourceName=' + sourceId + '&destinationName='
				+ destinationId + '&journeyDate=' + journeyDate + '&execuseme='
				+ exe + '&User=' + u);
	...
	if (xmlhttp.readyState == 4) {
		...
		if (xmlhttp.status == 200) {
			...
			if (rssentries.length != 0) {
				...
				avail(serviceNumber, sourceId, destinationId, busType,
						journeyDate, departureTime, arrivalTime, fare, routeId,
						check);
			} else {
				$("#load").hide();
...
function avail(serviceNumber, source, destination, busType, da, departureTime,
		arrivalTime, fare, routeId, check) {
	...
	start2(serv, serviceNumber, source, destination, busType, da,
			departureTime, arrivalTime, fare, routeId, check);
}
...
function start2(serv, serviceNumber, source, destination, busType, date,
		departureTime, arrivalTime, fare, routeId, check) {
	alert("start2");
	var j = serv;

	if (serv == 0) {
		start1(serviceNumber, source, destination, busType, date,
		...
		ajaxFunction1(servicenumber, source, destination, busType, date,
				departureTime, arrivalTime, fare2, routeId, check1);

	}

}

function start1(serviceNumber, source, destination, busType, date,
		departureTime, arrivalTime, fare) {
	alert("start1");
	$("#load").hide();
...
function ajaxFunction1(serviceNumber, source, destination, busType, date,
		departureTime, arrivalTime, fare, busId, check) {
...
function ajaxFunctions3(sourceId, destinationId, date, routeId, fare, checkv,
		busType) {
...
var my = 0;
var check_s = "";
function ajaxFunction(sourceId1, destinationId1, journeyDate1, busId1, fare1,
		check, busType) {
	...
	if (xmlhttp) {
				...
				if (xmlhttp.status == 200) {
					...
					if (rssentries.length > 0) {
						...
						for ( var m1 = 0; m1 < avail_seats.length; m1++) {
							if (avail_seats[m1] != null) {
								blck_seats.push(avail_seats[m1]);
							}
						}
						// Booked seats:
						for ( var m2 = 0; m2 < booked_seats1.length; m2++) {
							if (booked_seats1[m2] != null) {
								b_seats.push(booked_seats1[m2]);
							}
						}
						// ladies seats:
						for ( var m3 = 0; m3 < booked_seats_l.length; m3++) {
							if (booked_seats_l[m3] != null)
								b_seats_l.push(booked_seats_l[m3]);
						}
						...
						if (b_seats.length != 0) {
							if (b_seats_l.length != 0) {
								if (blck_seats.length != 0) {//alert("boarding8 "+boarding);
									if (isSleeper == true) {
										start_sleeper(b_seats, b_seats_l,
												blck_seats, fare, boarding,
												route, check);
									} else {
										start(b_seats, b_seats_l, blck_seats,
												fare, boarding, route, check);
									}
								} else {
		...
		//alert("name: "+name);
		dataType: "json";
		xmlhttp.send('action=Booking/getBusLayout&userName=' + name
				+ '&sourceName=' + sourceId + '&destinationName='
				+ destinationId + '&journeyDate=' + journeyDate + '&routeId='
				+ busId + '&execuseme=' + exe);
...
function start_sleeper(booked_seatsss, ladies, blck, fare, board, route, check) {
	...
	var l_slee = 0;
	document.getElementById("avail").src = CONTEXT_ROOT
			+ "/images/bus/sleeper/available.gif";
	document.getElementById("select").src = CONTEXT_ROOT
			+ "/images/bus/sleeper/selected.gif";
	document.getElementById("booked").src = CONTEXT_ROOT
			+ "/images/bus/sleeper/booked.gif";
	document.getElementById("blocked").src = CONTEXT_ROOT
			+ "/images/bus/sleeper/blocked.gif";
	...
	if (k == 0) {
		...
		do {
			...
			for ( var i = 1; i < rows; i++) {
				...
				if (i != 3) {
					...
					for ( var j = 1; j <= (col - 1); j++) {
						...
						s_cell.setAttribute("title", "Seat no."
								+ sleeper_seats[m - 1]);
						...
						if (alr_booked.length > 0 || agent_blocked.length > 0) {
							for ( var booked = 0; booked < alot; booked++) {
								...
								{
									if (check == 1) {
										s_cell.setAttribute("onclick",
												"booked_seat(" + check + ");");
										s_cell.style.backgroundImage = "url("
												+ img_book + ")";
									} else {
										s_cell.setAttribute("onclick",
												"booked_seat(" + check + ");");
										...
										if (m == agent_blocked[block]) {
											...
											for ( var l = 0; l < ladies.length; l++) {
												if (m == ladies[l]) {
													...
													s_cell.style.backgroundPosition = "5px 5px";
													break;
												} else {//var sea_t=new String(m);//alert("number==="+seat_no);//seat_no
													s_cell.setAttribute(
															"onclick",
															"sleeper_selected("
																	+ m + ");");
							...
							s_cell.setAttribute("onclick", "selected("
									+ seat_no + ");");
			...
			if (last_booked == false) {
				...
				cells.setAttribute("onclick", "sleeper_selected("
						+ (last_seat + 1) + ");");
			} else {
...
function booked_seat(check) {
	...
	ladie = lady;
	var isSleeper = false;
	var splitData = busTypes.split(" ");
	for ( var s = 0; s < splitData.length; s++) {
		if (splitData[s] == "Sleeper") {
			isSleeper = true;
		}
	}
	//alert(cou+" "+popup_data.length+" "+ladie);
	var g_seat;
	var Name;
	var PNR;
	var Mobile;
	...
	var ele;
	if ((cou % 2) == 0) {
		document.getElementById(check).innerHTML="<span id='click'> seatName:"+check+"<br/>Group Seat:"+g_seat+"<br/>Name: "+Name+"<br/>PNR No: "+PNR+"<br/>Mobile No: "+Mobile+"<br/>Booked By: "+Booked_by+"<br/>Route: "+Route+"<br/>Boarding Point: "+b_Point+"<br/></span>";
		if (isSleeper == false) {
			for ( var i = 0; i < (popup_data[(check - 1)].layoutPopupCTO).length; i++) {
				...
				if (i == 0) {
					ele = "<b>Route: " + Route + "</b><br/>seatName:" + check
							+ "<br/>Seat Status: " + seatStas
							+ "<br/>Group Seat:" + g_seat + "<br/>Name: "
							+ Name + "<br/>PNR No: " + PNR + "<br/>Mobile No: "
							+ Mobile + "<br/>Booked By: " + Booked_by
							+ "<br/>Boarding Point: " + b_Point;
				} else {
					ele = ele + "<br/><b>Route: " + Route
							+ "</b><br/>seatName:" + check
							+ "<br/>Seat Status: " + seatStas
							+ "<br/>Group Seat:" + g_seat + "<br/>Name: "
							+ Name + "<br/>PNR No: " + PNR + "<br/>Mobile No: "
			...
			document.getElementById(check).innerHTML = "<span id='click'>"
					+ ele + "</span>";
		} else {
			for ( var i = 0; i < (popup_data[(check - 1)].layoutPopupCTO).length; i++) {
				g_seat = popup_data[(check - 1)].layoutPopupCTO[i].groupSeat;
				...
				var seatStas = popup_data[(check - 1)].layoutPopupCTO[i].seatStatus;
				if (i == 0) {
					ele = "<b>Route: " + Route + "</b><br/>seatName:"
							+ sleeper_seats[check - 1] + "<br/>Seat Status: "
							+ seatStas + "<br/>Group Seat:" + g_seat
							+ "<br/>Name: " + Name + "<br/>PNR No: " + PNR
							+ "<br/>Mobile No: " + Mobile + "<br/>Booked By: "
							+ Booked_by + "<br/>Boarding Point: " + b_Point;
				} else {
					ele = ele + "<br/><b>Route: " + Route
							+ "</b><br/>seatName:" + sleeper_seats[check - 1]
							+ "<br/>Seat Status: " + seatStas
							+ "<br/>Group Seat:" + g_seat + "<br/>Name: "
							+ Name + "<br/>PNR No: " + PNR + "<br/>Mobile No: "
							+ Mobile + "<br/>Booked By: " + Booked_by
			...
			document.getElementById(sleeper_seats[check - 1]).innerHTML = "<span id='click'>"
					+ ele + "</span>";
		}
		cou++;
	} else {
		for ( var n = 0; n < ladie.length; n++) {
			...
			if (ladie[n] == check) {
				if (isSleeper == false) {
					document.getElementById(check).innerHTML = "<img src="
							+ img_ladies + " align='ridht'>";
				} else {
					document.getElementById(sleeper_seats[check - 1]).innerHTML = "<img src="
							+ img_ladies + " align='ridht'>";
				}
				break;
			} else {
...
function start(booked_seatsss, ladies, blck, fare, board, route, check) {
	...
	if (k == 0) {
		...
		var div = document.createElement("div");
		document.getElementById("avail").src = CONTEXT_ROOT
				+ "/images/bus/available.gif";
		document.getElementById("select").src = CONTEXT_ROOT
				+ "/images/bus/selected.gif";
		document.getElementById("booked").src = CONTEXT_ROOT
				+ "/images/bus/booked.gif";
				...
				for ( var j = 1; j <= col; j++) {
					...
					if (alr_booked.length > 0 || agent_blocked.length > 0) {
						for ( var booked = 0; booked < alot; booked++) {
							...
							{
								if (check == 1) {
									cell.setAttribute("onclick", "booked_seat("
											+ check + ");");
...
function block() {
	$("#txtMsg").html("You do not have permission to book this seat.");
	$('#warningModal').modal('show');
	//alert("You do not have permission to book this seat.");
}
...
function images(ladies, bus) {
	...
	if (bus == "sleeper") {
		for ( var l = 0; l < ladies.length; l++) {
			document.getElementById(sleeper_seats[ladies[l] - 1]).innerHTML = "<head><img src="
					+ img_ladies + " align='ridht'></head>";
		}
	} else {
		for ( var l = 0; l < ladies.length; l++) {
			document.getElementById(ladies[l]).innerHTML = "<head><img src="
					+ img_ladies + " align='ridht'></head>";
		}
	}
}
...
ladie=lady;
	...
	//document.getElementById(check).innerHTML="<span id='click'> seatName:"+check+"<br/>Group Seat:"+g_seat+"<br/>Name: "+Name+"<br/>PNR No: "+PNR+"<br/>Mobile No: "+Mobile+"<br/>Booked By: "+Booked_by+"<br/>Route: "+Route+"<br/>Boarding Point: "+b_Point+"<br/></span>";
			...
			for(var i=0;i<(popup_data[(check-1)].layoutPopupCTO).length;i++)
				{
	    	    	...
	    	    	if(i==0)
	    	    	{
	    	    		ele="<b>Route: "+Route+"</b><br/>seatName:"+check+"<br/>Seat Status: "+seatStas+"<br/>Group Seat:"+g_seat+"<br/>Name: "+Name+"<br/>PNR No: "+PNR+"<br/>Mobile No: "+Mobile+"<br/>Booked By: "+Booked_by+"<br/>Boarding Point: "+b_Point;
	    	    	}
	    	    	else
	    	    	{
			...
			for(var i=0;i<(popup_data[(check-1)].layoutPopupCTO).length;i++)
		{
	    	...
	    	if(i==0)
	    	{
	    		ele="<b>Route: "+Route+"</b><br/>seatName:"+sleeper_seats[check-1]+"<br/>Seat Status: "+seatStas+"<br/>Group Seat:"+g_seat+"<br/>Name: "+Name+"<br/>PNR No: "+PNR+"<br/>Mobile No: "+Mobile+"<br/>Booked By: "+Booked_by+"<br/>Boarding Point: "+b_Point;
	    	}
	    	else
	    	{
	    		ele=ele+"<br/><b>Route: "+Route+"</b><br/>seatName:"+sleeper_seats[check-1]+"<br/>Seat Status: "+seatStas+"<br/>Group Seat:"+g_seat+"<br/>Name: "+Name+"<br/>PNR No: "+PNR+"<br/>Mobile No: "+Mobile+"<br/>Booked By: "+Booked_by+"<br/>Boarding Point: "+b_Point;
	    	}
		}
	...
	if (seat_counter < 6) {
		if (book_seats.length > 0) {
			if (book_seats[book_seats.length + 1] != m) {
				...
				m1
						.setAttribute("onclick", "unvalidateselectedseat(" + m
								+ ");");
				book_seats.push(m);
			}
		} else {
		...
		$("#txtMsg").html("More than six seats are not allowed for single time");
		$('#warningModal').modal('show');
	}
...
function sleeper_selected(m11) {
	...
	if (seat_counter < 6) {//alert("comes "+book_seats.length);
		if (book_seats.length > 0) {
			if (book_seats[book_seats.length + 1] != sleeper_seats[m - 1]) {
				...
				m1.style.backgroundRepeat = "no-repeat";
				m1.setAttribute("onclick", "sleeper_unvalidateselectedseat("
						+ m + ");");
...
function seats_list(val) {
	...
	for ( var z1 = 0; z1 < arr.length; z1++) {
		...
		if (gender[z1] == '--SELECT--') {
			...
			document.getElementById(id).innerHTML = "Select Valid Gender on seat:"
					+ arr[z1];
	...
	if (num < 10) {
		for ( var m = 0; m <= lady.length; m++) {
			if (num == lady[m]) {
				...
				newdiv.innerHTML = '<div  style="margin-left:3%;"><table border="0"><tr><td width="25%"><span style="font-size:12px;margin-top:2%;">'
						+ num
						+ '</span></td><td><lable  style="margin-left:14%;"><select class="logg" id="gender'
						+ num
						+ '"style="width:120px;height:24px;font-size: 10px;margin-top:6%;"><option>--SELECT--</option><option>FEMALE</option></select></lable><input type="hidden"style="width:28px;height:15px;" value="0" id="age'
						+ num + '"></td></tr></table></div>';
				break;
				...
				newdiv.innerHTML = '<div  style="margin-left:3%;"><table border="0"><tr><td width="25%"><span style="font-size:12px;margin-top:2%;">'
						...
						+ '</span></td><td><lable style="margin-left:14%;"><select class="logg" id="gender'
						+ num
						+ '"style="font-size: 10px;width:120px;height:24px;margin-top:6%;"><option>--SELECT--</option><option>MALE</option><option>FEMALE</option></select></lable><input type="hidden" style="width:28px;height:15px;" value="0" id="age'
						+ num + '"></lable></td></tr></table></div>';
			} else {
				continue;
			}
		}
		...
		for ( var m = 0; m <= lady.length; m++) {
			if (num == lady[m]) {
				...
				newdiv.innerHTML = '<div  style="margin-left:2%;"><table border="0"><tr><td width="25%"><span style="font-size:12px;margin-top:2%;">'
						+ num
						+ '</span></td><td><lable style="margin-left:8%;"><select class="logg" id="gender'
						+ num
						+ '" style="font-size: 10px;width:120px;height:24px;margin-top:6%;"><option>--SELECT--</option><option>FEMALE</option></select></lable><input type="hidden" style="width:28px;height:15px;"  value="0" id="age'
						+ num + '"></td></tr></table></div>';
				break;
			} else if (m == ((lady.length - 1))) {
				if (isSleeper) {
...
function ajaxF(pwd, n_pwd, ex) {
	if (xmlhttp) {
		...
		xmlhttp.setRequestHeader('Content-Type',
				'application/x-www-form-urlencoded');
		xmlhttp.send("action=AdminLogin/changePassword&currentPassword=" + pwd
				+ "&newPassword=" + n_pwd + "&execuseme=" + ex);
...
function ticket_book() {
	...
	/*if (xmlhttp) {
		...
		xmlhttp.setRequestHeader('Content-Type',
				'application/x-www-form-urlencoded');
		xmlhttp.send("action=" + action + "&userName=" + userName + "&routeId="
				+ routeId + "&journeyDate=" + journeyDate + "&boardingName="
				+ bb + "&phoneno=" + phoneno + "&name=" + name + "&seatName="
				+ seatName + "&gender=" + gender + "&totalSeats=" + totalSeats
...
function handle11() {
	if (xmlhttp.readyState == 4) {
		if (xmlhttp.status == 200) {
			...
			var che = json1.passengerBookedCTO.length;
			var sts = json1.totalFare;
			if (che > 0) {
				admin_print(json1);
			} else if (parseInt(sts) == 94623) {
				alert("Sorry!, bus departured already, Ticket can not be booked.");
				document.getElementById('btn').disabled = false;
			} else if (parseInt(sts) == 94624) {
				alert("We are sorry!, \nyou are not allowed to book ticket at this time.");
				document.getElementById('btn').disabled = false;
			} else {
				alert("Booking problem,please check your balance and try again later.");
				document.getElementById('btn').disabled = false;
			}
		}
	}
...
function addElement1(bookId, seatName, name, gender) {
	...
	for ( var j = 1; j <= bookId.length; j++) {
		...
		newdiv.innerHTML = '<table border="0" width="600" align="center">'
				+ '<tr><td width="125px">' + j
				+ '</td><td width="125px" ><label>&nbsp&nbspLMS'
				+ bookId[j - 1]
				+ '</label></td><td width="105px"style="padding-left:30px;">'
				+ seatName[j - 1]
				+ '</td><td width="120px" style="padding-right:5px;">'
				+ name[j - 1] + '</td><td width="125px">' + gender[j - 1]
				+ '</td></tr></table>';
		ni.appendChild(newdiv);
	}
}

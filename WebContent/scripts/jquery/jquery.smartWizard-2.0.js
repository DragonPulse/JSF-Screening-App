    ...
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                function doForwardProgress(){
                    ...
                    } else if(curStepIdx == 1){
                  	  if(validateSecondWizardForm()){
                  		  if(steps.length <= nextStepIdx){
                                if(!options.cycleSteps){
                                  return false;
                                }                  
				...
				function validateFirstWizardForm() {
						var retValue = true;
						//var impactedProcessfields = $("input[name='impactedProcesses']").serializeArray(); 
						//var impactedInterfacefields = $("input[name='impactedInterfaces']").serializeArray(); 
						...
						/*if ($("#reqBy").val() == ''
								...
								|| $("#telNo").val() == '/[a-z]/i'
									|| $("#telNo").val().length < 10
									|| $("#telNo").val().length > 10) {
							$("#showValidationErrorMessageStep1").text(
			...
			function validateSecondWizardForm() {
			var retValue = false;
			var radioButton = document
					.getElementsByName("outageRequired");
			...
			if (!retValue) {
				$("#showValidationErrorMessageStep2").text(
						" ** Please enter all the mandatory fields.** ");
			} else {
				$("#showValidationErrorMessageStep2").text("");
			}
			return retValue;
				...
				function validateFourthWizardForm() {
					var retValue = true;
					/*if ($("#unitTestPlan").val() == ''
							|| $("#unitTestResult").val() == ''
							|| $("#intTestPlan").val() == ''
							|| $("#intTestResult").val() == ''
							|| $("#userAccptTestPlan").val() == ''
								|| $("#userAccptTestResult").val() == '') {
						$("#showValidationErrorMessageStep4").text(
								" ** Please enter all the mandatory fields.** ");
						retValue = false;
					} else {
						$("#showValidationErrorMessageStep4").text("");

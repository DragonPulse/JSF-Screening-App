	...
	$(function() {		
		...
		var onLoad = function(){
		 ...
		 var processAddEdit=function(response, postdata) {	
		    	...
		    	var appCodesURL="getAllMyCandidates.html";	  		
		  		$("#all-requirements-grid").jqGrid('setGridParam', {url:appCodesURL,datatype:'json',page:1}).trigger("reloadGrid");
			
		 }
		
		//declaration for hideLoading
		var hideLoading = function () {
    		jQuery("#lui_all-requirements-grid").hide();
    		jQuery("#load_all-requirements-grid").hide();
	 	};	 	
		...
		$(document).on('click', 'a[id^="view"]', function(event) {
				...
				var dataFromTheRow = jQuery('#all-requirements-grid').jqGrid ('getRowData', rowId);
		...
		jQuery("#all-requirements-grid").jqGrid({
					datatype : "json",
					rowNum : 10,
					url:"getAllRequirements.html",
					rowList : [ 10,15,20 ],
					colNames : [ '','Job Code','Job Designation','Client Name', 'Client Location','Duration','CreatedBy','CreatedDate','UpdatedBy','UpdatedDate','Edit', 'View'],
					colModel : [	
					{
						name : 'jobReqId',
						index : 'jobReqId',
					...
					},
					 {
						...
						name : 'client.clientContactName',
						index : 'client.clientContactName',
						width : '100%',
						align : 'center'
					}, {
						name : 'client.clientEmail',
						index : 'client.clientEmail',
						width : '100%',
						align : 'center'
					}, {
						name : 'createdBy',
					...
					pager : "#all-requirements-grid-pager",
					height : '250px',
					gridview : true,
					viewrecords : true,
					loadonce :true,
		...
		jQuery("#all-requirements-grid").jqGrid('filterToolbar', {
			stringResult : true,
			searchOnEnter : false
		});
		
		jQuery("#all-requirements-grid")
		.navGrid('#all-requirements-grid-pager',{del:false,edit:false,add:false,search:false,refresh:true},
	      { // edit options 
	        url: 'editUser.html',
	        closeAfterEdit: true,
